...
export default class LaserObstacle extends Phaser.GameObjects.Container {
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    const bottom = scene.add.image(0, middle.y + middle.displayHeight, TextureKeys.LaserEnd).setOrigin(0.5, 0).setFlipY(true);
    this.add(top);
    this.add(middle);
    this.add(bottom);
    ...
    body.setSize(width * 0.9, height);
    body.setOffset(-width * 0.45, -height * 0.4); // TODO: why offset is useless 
